...
let nome_informado = entrada.question('Digite o seu nome:');

//EXECUÇÃO LOGICA

let questoesEmbaralhadas = questoes.sort(() => Math.random() - 0.5);

let questoesSelecionadas = questoesEmbaralhadas.slice(0,10);

for(let questaoSelecionada of questoesSelecionadas){
    let resposta = entrada.question(questaoSelecionada.pergunta+" \n");
    if(resposta == questaoSelecionada.resposta){
